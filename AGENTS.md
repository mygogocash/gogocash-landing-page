...
Static-export Next.js 16 marketing site for GoGoCash (cashback rewards). No backend, no database — purely static pages served from Cloudflare Workers Static Assets in production. See `README.md` for full architecture and npm script reference.
...
| Production deploy | `npm run deploy:cloudflare:production` |
...
- **No environment variables are required** for local dev. All optional secrets (Involve Asia, Strapi, Cloudflare Web Analytics) fall back to sensible defaults. See `.env.example` for reference.
